...
class DnsApi
{
    ...
    public function listDefaultDnsRecordsWithHttpInfo(string $contentType = self::contentTypes['listDefaultDnsRecords'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\OpenAPI\Client\Model\DefaultDnsRecord[]' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\OpenAPI\Client\Model\DefaultDnsRecord[]' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\OpenAPI\Client\Model\DefaultDnsRecord[]', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }
