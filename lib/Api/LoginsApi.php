...
class LoginsApi
{
    ...
    public function getUiPreferencesWithHttpInfo(string $contentType = self::contentTypes['getUiPreferences'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\OpenAPI\Client\Model\UiPreferences' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\OpenAPI\Client\Model\UiPreferences' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\OpenAPI\Client\Model\UiPreferences', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }
